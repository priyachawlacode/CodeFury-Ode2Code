...
public class FriendsDaoImpl implements FriendsDao {
	...
	private static PreparedStatement getFriendList, getFriendList2, remFriend, getFrndReqList, isFrnd, sendFrndReq,
			accptFrndReq, accptFrndReq2, declineReq, isPending;
	...
	static {
		try {
			...
			isPending = conn.prepareStatement(
					"select * from friendListRequest where (user_id=? and friend_id=?) or (user_id=? and friend_id=?)");
	...
	public List<User> getFriendList(String userId) {
		...
		try {
			...
			getFriendList2.setString(1, userId);

			rs = getFriendList2.executeQuery();
			...
			while (rs.next()) {
				...
				u.setState(rs.getString("state"));
				u.setUserName(rs.getString("username"));
				//System.out.println("INside frnd daoimpl rec friend2: " + u.toString());
				friendList.add(u);
...
//		}
	...
	public boolean isFriend(FriendRequest request) {
		boolean b = true;
		try {
			isFrnd.setString(1, request.getUser_id());
			isFrnd.setString(2, request.getFriend_id());
			isFrnd.setString(3, request.getFriend_id());
			isFrnd.setString(4, request.getUser_id());
	...
	@Override
	public boolean isPending(FriendRequest request) {
		try {
			isPending.setString(1, request.getUser_id());
			isPending.setString(2, request.getFriend_id());
			isPending.setString(3, request.getFriend_id());
			isPending.setString(4, request.getUser_id());
