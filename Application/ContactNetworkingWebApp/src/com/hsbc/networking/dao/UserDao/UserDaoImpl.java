...
public class UserDaoImpl implements UserDao {
	...
	private static PreparedStatement addUser, upUser, getUserDet, dctvUser, getUserByMail;
	...
	static {
		try {
			...
			getUserByMail = conn.prepareStatement("select * from users where emailId=?");

			dctvUser = conn.prepareStatement("update users set isDeactivated = 1 where username= ?");
	...
	public boolean deactivateUser(String username) {
		// System.out.println("REcieved userame to deactive: "+username);
...
//		}
	...
	public User checkUserMailIdExists(String emailId) {
		User u = null;
		try {
			getUserByMail.setString(1, emailId);
