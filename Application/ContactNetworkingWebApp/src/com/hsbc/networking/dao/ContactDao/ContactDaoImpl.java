...
public class ContactDaoImpl implements ContactDao {
	...
	private static PreparedStatement addContact, upContact, getContact, dltContact, mailIdExists, addContactList,
			getAllContacts, getContactFilter, getUser,dltContactList,getAllContactsSorted;
	...
	static {
		try {
			...
			dltContactList = conn.prepareStatement("delete from contactlist where contact_id=?");
	...
	public boolean deleteContact(String contactId, String userId) throws ContactNotFoundException {
		...
		try {
			...
			dltContactList.setString(1, contactId);
	...
	public boolean deleteContactsInBatch(List<String> contactIds, String userId)
		...
		for (int i = 0; i < contactIds.size(); i++) {
			...
			dltContactList.setString(1, contactIds.get(i));
			dltContact.addBatch();
			dltContactList.addBatch();
		}
		int successCount[] = dltContact.executeBatch();
		dltContactList.executeBatch();
	...
	public Contact checkContactMailIdExists(String userId, String emailId) {
		System.out.println("userId "+userId+" EmailId "+emailId);
		ResultSet resultSet = null;
		Contact c;
		c=null;
		try {
...
//		System.out.println(c.toString());
	...
	public List<Contact> getAllContactsSorted(String userId, String sortBy, boolean ascending) {
		String value = ascending ? "asc" : "desc";
		System.out.println("Values receved :"+userId+" "+sortBy+" "+value);
		ResultSet resultSet = null;
		List<Contact> cList = new ArrayList<Contact>();
		try {
			getAllContactsSorted = conn.prepareStatement(
					"select contactlist.contact_id, fullname, emailId, contactNo,gender,dob, address,city, state, country, company , profileImage from contactlist  join contactdetails on contactlist.contact_id = contactdetails.contact_id where user_id =? order by "+sortBy+" " +value);
			getAllContactsSorted.setString(1, userId);
//			getAllContactsSorted.setString(2, sortBy);
//			getAllContactsSorted.setString(3, value );
			resultSet = getAllContactsSorted.executeQuery();
