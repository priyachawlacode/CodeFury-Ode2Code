...
public class UserAuthImpl implements UserAuthDao {
	...
	static {
		try {
			...
			deleteFromContact = conn.prepareStatement("delete from contactdetails where contactdetails.contact_id in( select contact_id from contactlist where user_id=?)");
	...
	public boolean deleteUserAuth(String id) {
		...
		boolean userDeleted = deleteAllContacts(id);
		try {
			dltUser.setString(1, id);
	...
	public boolean deleteInBatch(List<String> userIdList) throws SQLException {
		...
		int successCount[] = dltUser.executeBatch();
