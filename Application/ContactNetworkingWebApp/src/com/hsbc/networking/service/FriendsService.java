...
public class FriendsService {
	...
	public void sendFriendRequest(FriendRequest request) throws RequestAlreadyPendingException, FriendBlockedException {
		...
		if (block.isBlocked(request.getUser_id(), request.getFriend_id())) {
			throw new FriendBlockedException("You are blocked by this person");
			
		}
		if (friendsDao.isPending(request.getUser_id(),request.getFriend_id())) {
			throw new RequestAlreadyPendingException("Request is already Pending");
	...
	public boolean isFriend(String userId, String reqId) {
		return friendsDao.isFriend(userId,reqId);
	}
	...
	public boolean isPending(String userId, String reqId) {
		return friendsDao.isPending(userId,reqId);
	}
